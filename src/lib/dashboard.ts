...
import { autoExpireQuotes } from "@/lib/quotes";
  ...
  const [
    ...
    prisma.shipment.count({
      where: {
        companyId,
        status: {
          in: [ShipmentStatus.OPEN, ShipmentStatus.IN_TRANSIT, ShipmentStatus.ARRIVED, ShipmentStatus.CUSTOMS],
        },
      },
    }),
    prisma.shipmentMilestone.count({
      where: { shipment: { companyId }, status: MilestoneStatus.DELAYED },
    }),
    prisma.quote.count({ where: { companyId } }),
    prisma.quote.count({ where: { companyId, status: QuoteStatus.APPROVED } }),
    prisma.quote.count({
      where: { companyId, status: { in: [QuoteStatus.APPROVED, QuoteStatus.REJECTED] } },
    }),
    prisma.quote.findMany({
      where: { companyId, status: QuoteStatus.APPROVED },
      select: { grossMarginAmount: true },
    }),
  ]);

  const totalApprovedMargin = approvedQuoteMargins.reduce(
    (acc, q) => acc + Number(q.grossMarginAmount ?? 0), 0,
  );

  const approvalRate = allQuotesDecided > 0
