...
type FinanceInvoiceEditorProps = {
  mode: "create" | "edit";
...
function defaultLine(): InvoiceLineInput {
  return {
    id: lineId(),
    description: "",
    amount: "",
...
export function FinanceInvoiceEditor({
  mode,
  shipments,
  initialValue,
}: FinanceInvoiceEditorProps) {
  const router = useRouter();
  const [selectedShipmentId, setSelectedShipmentId] = useState(
    initialValue?.shipmentId ?? shipments[0]?.id ?? "",
  );
  const [lines, setLines] = useState<InvoiceLineInput[]>(
    initialValue?.lines.length
  ...
  const action = mode === "create" ? createFinanceInvoiceAction : updateFinanceInvoiceAction;
  ...
  return (
    <form action={formAction} className="space-y-4 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
      {mode === "edit" && initialValue ? (
        <input type="hidden" name="invoiceId" value={initialValue.invoiceId} />
      ) : null}
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <div>
          ...
          <select
            name="currencyCode"
            defaultValue={initialValue?.currencyCode ?? "USD"}
            className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none transition focus:border-sky-300 focus:bg-white"
          >
        ...
        <div>
          ...
          <input
            type="date"
            name="issueDate"
            defaultValue={initialValue?.issueDate ?? ""}
            className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none transition focus:border-sky-300 focus:bg-white"
          />
        </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {state.error && !state.success ? <p className="text-sm text-rose-700">{state.error}</p> : null}
      </div>
    </form>
  );
}
